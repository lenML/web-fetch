...
describe("transform_content", () => {
   ...
   */
  it("should transform small html to inline markdown", async () => {
    const result = await transform_content("<h1>Hi</h1>", "text/html");
   ...
   */
  it("should transform small json to inline output", async () => {
    const result = await transform_content('{"a":1}', "application/json");
    expect(result.inline).toBe(true);
   ...
   */
  it("should return inline raw text for unknown small content", async () => {
    const result = await transform_content("plain text", "text/plain");
    expect(result.inline).toBe(true);
   ...
   */
  it("should chunk large html and save to temp store", async () => {
    ...
    const result = await transform_content(html, "text/html");
   ...
   * Then it should chunk and save to temp store
   */
  it("should chunk large json and save to temp store", async () => {
    ...
    const result = await transform_content(json, "application/json");
    expect(result.inline).toBe(false);
    expect(result.fetch_record).toBeDefined();
